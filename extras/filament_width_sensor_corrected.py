# Support for filament width sensor with correction
#
# Based on hall_filament_width_sensor.py by Mustafa YILDIZ <mydiz@hotmail.com>
# Extended with diameter offset and correction factor for fine-tuning
#
# Copyright (C) 2019  Mustafa YILDIZ <mydiz@hotmail.com>
# Copyright (C) 2025  Extended version
#
...
class FilamentWidthSensorCorrected:
    def __init__(self, config):
        ...
        # Correction parameter for fine-tuning without recalibration
        self.diameter_offset = config.getfloat('diameter_offset', 0.0)
        ...
        # Register commands (with _CORRECTED suffix to avoid conflicts)
        self.gcode = self.printer.lookup_object('gcode')
        self.gcode.register_command('QUERY_FILAMENT_WIDTH_CORRECTED', 
                                    self.cmd_M407)
        self.gcode.register_command('RESET_FILAMENT_WIDTH_SENSOR_CORRECTED',
                                    self.cmd_ClearFilamentArray)
        self.gcode.register_command('DISABLE_FILAMENT_WIDTH_SENSOR_CORRECTED',
                                    self.cmd_M406)
        self.gcode.register_command('ENABLE_FILAMENT_WIDTH_SENSOR_CORRECTED',
                                    self.cmd_M405)
        self.gcode.register_command('QUERY_RAW_FILAMENT_WIDTH_CORRECTED',
                                    self.cmd_Get_Raw_Values)
        self.gcode.register_command('ENABLE_FILAMENT_WIDTH_LOG_CORRECTED',
                                    self.cmd_log_enable)
        self.gcode.register_command('DISABLE_FILAMENT_WIDTH_LOG_CORRECTED',
                                    self.cmd_log_disable)
        self.gcode.register_command('SET_FILAMENT_WIDTH_OFFSET',
                                    self.cmd_set_offset)
    ...
    def adc2_callback(self, read_time, read_value):
        ...
        # Apply correction offset for fine-tuning
        diameter_new = diameter_new + self.diameter_offset
    ...
    def cmd_set_offset(self, gcmd):
        ...
        if offset is None:
            gcmd.respond_info("Current diameter offset: %.4f mm" % 
                            self.diameter_offset)
        else:
            self.diameter_offset = offset
            gcmd.respond_info("Diameter offset set to: %.4f mm" % offset)
            gcmd.respond_info("Example: Sensor shows 1.72mm, real is 1.732mm")
            gcmd.respond_info("         Offset = 1.732 - 1.72 = +0.012mm")

def load_config(config):
    return FilamentWidthSensorCorrected(config)
