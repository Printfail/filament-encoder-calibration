...
bool FilamentTracker::update() {
    ...
    // FILTER: Gleitender Durchschnitt über letzte Messungen
    uint16_t filtered_angle = get_filtered_angle(current_angle);
    
    // Erkenne Überläufe (0→16383 oder 16383→0)
    detect_overflow(filtered_angle);
    
    // Berechne Delta mit gefiltertem Wert
    int32_t delta = static_cast<int32_t>(filtered_angle) - static_cast<int32_t>(last_angle_);
...
void FilamentTracker::detect_overflow(uint16_t current_angle) {
    const uint16_t THRESHOLD = AS5048A::RESOLUTION / 4;  // 4096 steps
    
    int32_t diff = static_cast<int32_t>(current_angle) - static_cast<int32_t>(last_angle_);
    
    // Vorwärts-Überlauf: 16000 → 100 (diff = -15900, aber eigentlich +484)
    if (diff < -static_cast<int32_t>(THRESHOLD)) {
        revolutions_++;
        printf("FilamentTracker: Forward overflow detected (rev=%d)\n", revolutions_);
    }
    // Rückwärts-Überlauf: 100 → 16000 (diff = +15900, aber eigentlich -484)
    else if (diff > static_cast<int32_t>(THRESHOLD)) {
        revolutions_--;
